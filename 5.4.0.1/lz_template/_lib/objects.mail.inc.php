        ...
        try
        {
            ...
            if(strpos($this->Receiver,",") !== false)
            {
                ...
                foreach($emails as $mailrec)
                    if(!empty($mailrec) && !isnull(strpos($mailrec,"@")))
                    {
                        if(!$add)
                        {
                            $add = true;
            ...
            else if(!isnull(strpos($this->Receiver,"@")))
    ...
    private function SEND_PHP_MAIL($_receiver="",$result = "",$_html=false)
    {
        if(strpos($_receiver,",") !== false)
        {
            ...
            foreach($emails as $mail)
                if(!empty($mail) && !isnull(strpos($mail,"@")))
                ...
                try
                {
                    ...
                    foreach ($parts as $part)
                    {
                        try
                        {
                            ...
                            if(DEBUG_MODE)logit(" PROCESSING EMAIL / charset:" . $ctype . " - " . $charset . " - " . $subject . " - " . $isatt);
                            ...
                            if (!$isatt && (($html = (strpos(strtolower($ctype), 'text/html') !== false)) || strpos(strtolower($ctype), 'text/plain') !== false))
                            {
                                ...
                                if ((!$html || empty($temail->Body)) && !empty($content))
                                {
                                    ...
                                    {
                                        if(DEBUG_MODE)logit(" PROCESSING EMAIL / iconv | " . strtoupper($charset) . " | " . 'UTF-8' . " | " . $subject);
                            ...
                            else
                            {
                                ...
                                if(DEBUG_MODE)logit("ADD ATT: " . $filesid . " - " . $filename . " - " . $ctype);
                            }
                        }
                        catch (Exception $e)
                        {
