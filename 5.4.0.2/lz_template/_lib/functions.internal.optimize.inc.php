...
function maintainTables($_randStandard,$_timeouts)
{
    ...
    {
        queryDB(true,"UPDATE `".DB_PREFIX.DATABASE_RESOURCES."` SET `discarded`=1,`edited`=UNIX_TIMESTAMP() WHERE NOT EXISTS(SELECT * FROM `".DB_PREFIX.DATABASE_TICKET_ATTACHMENTS."` WHERE `res_id` = `".DB_PREFIX.DATABASE_RESOURCES."`.`id`) AND `discarded`=0 AND `type`=3 AND `parentid`=100 LIMIT 10;");
