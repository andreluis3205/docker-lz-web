...
ChatTranslationEditorClass.prototype.showTranslationEditor = function(languages) {
    ...
    var footerString = lzm_displayHelper.createButton('save-translation-editor', 'ui-disabled', '', t('Save'), '', 'lr',{'margin-left': '6px'},'',30,'d') +
        lzm_displayHelper.createButton('cancel-translation-editor', '', '', t('Cancel'), '', 'lr',{'margin-left': '6px'},'',30,'d');
...
ChatTranslationEditorClass.prototype.addTranslationLanguage = function(type, myTab) {
    ...
    var headerString = t('Translation Editor');
    var footerString = lzm_displayHelper.createButton('save-translation-language-add', '', '', t('Save'), '', 'lr',{'margin-left': '6px'},'',30,'d') +
        lzm_displayHelper.createButton('cancel-translation-language-add', '', '', t('Cancel'), '', 'lr', {'margin-left': '6px'},'',30,'d');
...
ChatTranslationEditorClass.prototype.createTranslationEditorHtml = function(myTab) {
    ...
    langSelHtml += '</tbody></table>' +
        ...
        '<div style="margin: 14px 0px;">' + lzm_displayHelper.createButton(idPrefix + 'translation-language-new', 'translation-lang-btn', 'addTranslationLanguage(\'' + myTab + '\')', t('Add'), '', 'lr', {}, t('Add Language'),30,'e') + '</div>' +
        '<div style="margin: 14px 0px;">' + lzm_displayHelper.createButton(idPrefix + 'translation-language-delete', 'translation-lang-btn ui-disabled', 'deleteTranslationLanguage(\'' + myTab + '\')', t('Delete'), '', 'lr', {}, t('Delete Language'),30,'e') + '</div>';
    langSelHtml += '<div style="margin: 14px 0px;">' + lzm_displayHelper.createButton(idPrefix + 'translation-language-suggest', 'translation-lang-btn', 'suggestTranslationLanguage(\'' + myTab + '\')', t('Suggest as default'), '', 'lr', {}, t('Suggest translation as default'),30,'e') + '</div>' +
        '<div style="margin: 14px 0px;">'
        + lzm_displayHelper.createButton(idPrefix + 'translation-language-download', 'translation-lang-btn', 'downloadTranslationLanguage(\'' + myTab + '\')', t('Download default'), '', 'lr', {}, t('Download default translation'),30,'e') + '</div>';
...
ChatTranslationEditorClass.prototype.createTranslationLanguageEditorHtml = function() {
    ...
    for (i=0; i<availLangCodes.length; i++) {
        if ($.inArray(availLangCodes[i], existingLangCodes) == -1) {
            ...
            //optHTML += '<option value="' + availLangCodes[i] + '">' + availLangCodes[i].toUpperCase() + ' - ' +
              //  lzm_chatDisplay.availableLanguages[availLangCodes[i]] + '</option>';
        }
    }

    myHtml += lzm_inputControls.createSelect('language-selection', '', '', true, {position: 'right', gap: '0px'}, {}, '', langArray, '', '');
