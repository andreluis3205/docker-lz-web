...
ChatVisitorClass.prototype.createVisitorListLine = function(aUser, visitorListWidth, newLine) {
    ...
    var tableRowTitle = '';
    var visitorName = (that.createVisitorStrings('cname', aUser).length > 32) ? that.createVisitorStrings('cname', aUser).substring(0, 32) + '...' : that.createVisitorStrings('cname', aUser);
    var visitorEmail = (that.createVisitorStrings('cemail', aUser).length > 32) ? that.createVisitorStrings('cemail', aUser).substring(0, 32) + '...' : that.createVisitorStrings('cemail', aUser);
    ...
    var visitorCompany = (that.createVisitorStrings('ccompany', aUser).length > 32) ? that.createVisitorStrings('ccompany', aUser).substring(0, 32) + '...' : that.createVisitorStrings('ccompany', aUser);
    ...
    extUserHtmlString += '<td class="icon-column nobg"><div style="margin-top:-1px;background-image: url(\'./php/common/flag.php?cc=' + aUser.ctryi2 + '\');" class="visitor-list-flag"></div></td>';
    if (visitorIsChatting && !visitorWasDeclined) {
        extUserHtmlString += '<td class="icon-column nobg" nowrap style="padding-top: 2px;";>' +
    ...
    extUserHtmlString += '<td class="icon-column nobg" nowrap>'+visitorInvitationFont+'</td>';
    for (i=0; i<lzm_chatDisplay.mainTableColumns.visitor.length; i++) {
...
ChatVisitorClass.prototype.updateVisitorInformation = function(thisUser) {
    var that = this;
    $('#visitor-details-list').html(that.createVisitorInformation(thisUser)).trigger('create');
    ...
    $('#visitor-comment-list').html(that.createVisitorCommentTable(thisUser)).trigger('create');
    $('#visitor-invitation-list').html(that.createVisitorInvitationTable(thisUser)).trigger('create');
...
ChatVisitorClass.prototype.showVisitorInformation = function (thisUser, chatId, activeTab) {
    ...
    var footerString = lzm_displayHelper.createButton('cancel-visitorinfo', '', '', t('Close'), '', 'lr',{'margin-left': '4px'},'',30,'d');
    var bodyString = '<div id="visitor-info-placeholder"></div>';
    var dialogData = {'visitor-id': thisUser.id, menu: t('Visitor Information: <!--name-->', [['<!--name-->', visitorName]]), 'chat-type': '1', 'reload': ['chats', 'tickets'], ratio: lzm_chatDisplay.DialogBorderRatioFull};
    ...
    var detailsHtml = '<div id="visitor-details-list" style="overflow-y: auto;" data-role="none">' + that.createVisitorInformation(thisUser) + '</div>';
    var historyHtml = '<div id="visitor-history-list" data-role="none"><div id="visitor-history-placeholder"></div></div>';
    var commentText = '', commentsHtml = '';
    try {
        commentText = (thisUser.c.length > 0) ? thisUser.c[0].text.replace(/\r\n/g, '\n').replace(/\r/g, '\n').replace(/\n/g, '<br />') : '';
    } catch(e) {}
    commentsHtml = '<div id="visitor-comment-list" data-role="none">' + that.createVisitorCommentTable(thisUser) + '</div>';
    var invitationsHtml = '<div id="visitor-invitation-list" data-role="none">' +
        that.createVisitorInvitationTable(thisUser) +
        '</div>';
    var chatsHtml = '<div style="position:static;" class="lzm-dialog-headline2"><span style="float:right;padding-top:5px;margin-right:4px;">' +
        lzm_displayHelper.createButton('create-ticket-from-chat', '', '', t('Create Ticket'),'<i class="fa fa-plus"></i>', 'lr', {'margin-left': '4px'}, '', 20) +
        lzm_displayHelper.createButton('send-chat-transcript', '', '', t('Send transcript to...'), '<i class="fa fa-mail-forward"></i>', 'lr',{'margin-left': '4px'}) +
        lzm_displayHelper.createButton('link-with-ticket', '', '', t('Link with Ticket'),'<i class="fa fa-link"></i>', 'lr', {'margin-left': '4px', 'margin-right': '4px'}) +'</span></div>'
        + lzm_chatDisplay.archiveDisplay.createMatchingChats(chatId) +
        '<div style="position:static;height:1px;" class="lzm-dialog-headline2" data-role="none"></div>'+
        '<div id="chat-content-inner-div"><div data-role="none" id="chat-content-inner"></div></div>';
    var ticketsHtml = lzm_chatDisplay.ticketDisplay.createMatchingTickets() +
        '<fieldset class="lzm-fieldset" data-role="none" id="ticket-content-inner"></fieldset>';
    ...
    if (typeof thisUser.b != 'undefined') {
        ...
        coBrowseHtml = '<div class="lzm-fieldset" data-role="none" id="visitor-cobrowse">' +
        ...
        coBrowseHtml += '<div style="margin-top: 6px;"><select id="visitor-cobrowse-action-select" class="lzm-select' + disabledClass + '" data-role="none">' +
            ...
            '</select></div>' +
            '<div style="margin-top: 10px;"><label id="visitor-cobrowse-language-label" for="visitor-cobrowse-language-select" style="margin-right: 10px;text-align:left;">' +
            t('Language:') + '</label>' +
        ...
        coBrowseHtml += '</select></div></div></div>';
    }
    ...
    if (typeof thisUser.rv != 'undefined') {
        for (i=0; i<thisUser.rv.length; i++) {
            ...
            var recentHistoryHtml = '<div id="recent-history-' + thisUser.rv[i].id + '" class="browser-history-container" style="overflow-y: auto;"><div class="lz_anim_loading"></div></div>';
    ...
    if (typeof thisUser.id != 'undefined' && thisUser.id != '' && typeof thisUser.b_id != 'undefined') {
        ...
        visitorInfoArray = {
            ...
            location: {title: t('Location'), rows: [
                ...
                {title: t('Country'), content: '<div style="background-image: url(\'./php/common/flag.php?cc=' + thisUser.ctryi2 + '\');display:inline-block;" class="visitor-list-flag"></div>&nbsp;&nbsp;' + thisUser.ctryi2},
        ...
        if (visitorIsChatting && !visitorWasDeclined) {
            ...
            visitorInfoArray.misc.rows.push({title: tid('chatting_with'), content: chatPartnerNames.join(', ')});
        }
    } else {
        visitorStatus = t('<!--status_style_begin-->Offline<!--status_style_end-->',[
            ['<!--status_style_begin-->','<span style="color:#aa0000; font-weight: bold;">'],['<!--status_style_end-->','</span>']
    ...
    for (var myKey in visitorInfoArray) {
        if (visitorInfoArray.hasOwnProperty(myKey)) {
            ...
            for (var k=0; k<visitorInfoArray[myKey].rows.length; k++) {
                ...
                visitorInfoHtml += '<tr>' +
                    '<td style="text-align: left; width: 150px; padding-left:10px; font-weight: bold;" nowrap>' + visitorInfoArray[myKey].rows[k].title + '</td>' +
...
ChatVisitorClass.prototype.createBrowserHistory = function (visitor, rv) {
    ...
    var browserHistoryHtml = '<div' + containerDivId + ' class="browser-history-container" style="overflow-y: auto;height:100%;">' +
        '<table class="browser-history visitor-list-table alternating-rows-table lzm-unselectable">' +
        '<thead><tr>' +
    ...
    try {
        ...
        for (var i = 0; i < myB.length; i++) {
            if (myB[i].id.indexOf('OVL') == -1 && myB[i].h2.length > 0) {
                browserHistoryHtml += '<tr class="split_line"><td colspan="7">'+tid('browser') + ' ' + (i+1)+'</td></tr>';
...
ChatVisitorClass.prototype.createVisitorCommentTable = function(visitor) {
   ...
   // var selectedRow = (typeof $('#visitor-comment-list').data('selected-row') != 'undefined') ?  $('#visitor-comment-list').data('selected-row') : 0;
    var commentTableHtml = '<div class="lzm-dialog-headline3">' + lzm_displayHelper.createButton('add-comment', '', 'addVisitorComment(\'' + visitor.id + '\', \'' + menuEntry + '\')', t('Add Comment'), '<i class="fa fa-plus"></i>', 'lr', {'margin-right':'4px',float:'right'}, t('Add Comment'),'','b') + '</div><div id="visitor-comment-list-frame" style="overflow-y:auto;"><table class="visitor-list-table alternating-rows-table lzm-unselectable" id="visitor-comment-table" style="width: 100%;"><tbody>';
    try {
...
/*
            var invIcon = '<i class="fa fa-commenting icon-flip-hor"></i>';
            if (visitor.r[i].s != '' && visitor.r[i].a == '0' && visitor.r[i].ca == '' && visitor.r[i].de == '0') {
                invIcon = '<i class="fa fa-commenting icon-flip-hor icon-orange"></i>';
            } else if (visitor.r[i].s != '' && visitor.r[i].a == '1' && visitor.r[i].ca == "") {
                invIcon = '<i class="fa fa-commenting icon-flip-hor icon-green"></i>';
            }
...
ChatVisitorClass.prototype.showVisitorInvitation = function(aVisitor) {
    ...
    var text = '';
    var footerString = lzm_displayHelper.createButton('send-invitation', 'ui-disabled', '', t('Ok'), '', 'lr',{'margin-left': '4px'},'',30,'d') +
        lzm_displayHelper.createButton('cancel-invitation', '', '', t('Cancel'), '', 'lr',{'margin-left': '4px'},'',30,'d');
...
ChatVisitorClass.prototype.createVisitorInvitation = function(visitor) {
    ...
    var languageSelectHtml = '<label for="language-selection">' + t('Language:') + '</label>' +
    ...
    var groupSelectHtml = '<label for="group-selection">' + t('Group:') + '</label>' +
    ...
    var browserSelectHtml = '<label for="browser-selection" class="top-space">' + t('Browser:') + '</label>' +
    ...
    var textInputHtml = '<label for="invitation-text" class="top-space">' + t('Invitation text:') + '</label>' +
...
ChatVisitorClass.prototype.showFilterCreation = function(visitor, filter, inDialog) {
    ...
    var footerString = lzm_displayHelper.createButton('save-filter', '', '', t('Ok'), '', 'lr',{'margin-left': '4px'},'',30,'d') + lzm_displayHelper.createButton('cancel-filter', '', '', t('Close'), '', 'lr',{'margin-left': '4px'},'',30,'d');
    ...
    $('#save-filter').click(function() {
        ...
        $('#filter-list-loading').css({position: 'absolute', left: '0px', top: '0px', bottom: 0, right:0,
            'background-color': '#ffffff', 'z-index': 1000, opacity: 0.85});
    });
};
...
ChatVisitorClass.prototype.createVisitorFilterMainHtml = function(visitor, filter) {
    ...
    var filterHtml = '<fieldset class="lzm-fieldset" data-role="none" id="visitor-filter-main">' +
        ...
        '<td></td>' +
        '<td style="padding-bottom: 10px;">' +
        '<span><input style="vertical-align: middle;" type="checkbox" class="checkbox-custom" data-role="none" id="filter-active" '+filterActive+' />' +
        '<label for="filter-active" class="checkbox-custom-label">' + t('Filter active') + '</label></span></td>' +
        '</tr>' +
        '<tr>' +
        ...
        '<td style="padding-bottom: 10px;">' +
        '<input type="text" data-role="none" class="lzm-filter-input-main' + visitorIpDisabled + '" id="filter-ip" value="' + visitorIp + '" /><div class="lzm-info-text top-space-half">' +
        t('Use * as wildcard') + '</div></td>' +
        '</tr>' +
        '<tr>' +
        '<td colspan="2" style="text-align: center;"><label>' + t('- OR -') + '</label></td></tr>' +
        '<tr>' +
        ...
        '<br /><div class="lzm-info-text top-space-half">' + t('comma separated, <!--link-->', [['<!--link-->', iso639Link]]) + '</div></td>' +
        '</tr>' +
        '<tr>' +
        '<td colspan="2" style="text-align: center;"><label>' + t('- OR -') + '</label></td>' +
        '</tr>' +
        '<tr>' +
        '<td style="padding-bottom: 10px; white-space: nowrap;">' +
        ...
        '</span><div class="lzm-info-text top-space-half">' + t('comma separated, <!--link-->', [['<!--link-->', iso3166Link]]) + '</div></td>' +
        '</tr>' +
        '</table>' +
        ...
        '<label for="filter-monitoring-check" class="checkbox-custom-label">' + t('Monitoring / Tracking') + '</label>' +
        '</span></td></tr>' +
        '</table></fieldset>';
    return filterHtml;
};
...
ChatVisitorClass.prototype.showFilterList = function() {
    ...
    var footerString = lzm_displayHelper.createButton('add-filter-btn', disabledClass, '', t('New Filter'), '<i class="fa fa-plus"></i>', 'lr',{'margin-left': '4px'},'',30,'d') +
        lzm_displayHelper.createButton('close-filters', '', '', t('Close'), '', 'lr',{'margin-left': '4px'},'',30,'d');
    var dialogData = {};
...
ChatVisitorClass.prototype.createFilterListHtml = function() {
    ...
    var bodyString = '<table class="visitor-list-table alternating-rows-table lzm-unselectable" id="filters-table"><thead><tr>' +
        '<th></th>' +
        '<th></th>' +
    ...
    for (var i=0; i<filterList.length; i++) {
        ...
        bodyString += '<tr class="filters-list-line" id="filters-list-line-' + filterList[i].filterid + '" style="cursor: pointer;"' +
            ...
            '<td class="icon-column">'+bwIcon+'</td>' +
...
ChatVisitorClass.prototype.showTranslateOptions = function(visitorChat, language) {
    ...
    var bodyString = '<div id="translate-options-placeholder"></div>';
    var footerString =  lzm_displayHelper.createButton('save-translate-options', '', '', t('Ok'), '', 'lr',{'margin-left': '4px'},'',30,'d') +
        lzm_displayHelper.createButton('cancel-translate-options', '', '', t('Cancel'), '', 'lr',{'margin-left': '4px'},'',30,'d');
    var dialogData = {};
...
ChatVisitorClass.prototype.createVisitorListContextMenu = function(myObject) {
    ...
    contextMenuHtml += '<div>' +
        '<span id="show-this-visitor-details" class="cm-line cm-click" onclick="showVisitorInfo(\'' + myObject.visitor.id + '\');removeVisitorListContextMenu();">' +
    ...
    contextMenuHtml += '<div' + disabledClass + '>' +
        '<span id="invite-this-visitor" class="cm-line cm-click" onclick="' + onclickAction + '">' + invText + '</span></div>';
    ...
    contextMenuHtml += '<div' + disabledClass + '>' +
        ...
        '<span id="start-chat-this-visitor" class="cm-line cm-line-icon-left cm-click" onclick="' + onclickAction + '">' + t('Start Chat') + '</span></div><hr />';
...
ChatVisitorClass.prototype.createFilterListContextMenu = function(myObject) {
    ...
    contextMenuHtml += '<div>' +
        ...
        '<span id="new-filter" class="cm-line cm-line-icon-left cm-click" onclick="showFilterCreation(\'\', \'\', \'\', true);">' +
        t('New Filter') + '</span></div>';
    contextMenuHtml += '<div>' +
        '<i class="fa fa-edit"></i>' +
        '<span id="edit-filter" class="cm-line cm-line-icon-left cm-click" onclick="showFilterCreation(\'\', \'\', \'' + myObject.filterid + '\', true);">' +
