...
ChatDisplayHelperClass.prototype.createAddToDynamicGroupHtml = function(id, browserId) {
    ...
    var dynGroupHtml = '<form><fieldset data-role="none" id="add-to-group-form" class="lzm-fieldset"><legend>' + t('Add to existing group') + '</legend>' +
        ...
        '<table id="dynamic-group-table" class="visitor-list-table alternating-rows-table lzm-unselectable" data-selected-group="' + firstGroupId + '"><tbody>' + tableLines + '</tbody></table></div></fieldset>' +
    ...
    for (i=0; i<viewSelectArray.length; i++) {
        ...
        switch(viewSelectArray[i].id) {
            ...
            case 'world':
                ...
                break;
            case 'mychats':
                var chatnum = lzm_chatDisplay.myChatsCounter + '/' + lzm_chatDisplay.allchatsDisplay.chatCounter.q;
...
ChatDisplayHelperClass.prototype.createMainMenuPanel = function() {
    ...
    var panelHtml = '<div id="main-menu-panel-status" style="position: absolute; left: 0px; top: 0px; cursor: pointer; border-top-left-radius: inherit;' +
        ...
        '<div id="main-menu-panel-settings" class="lzm-unselectable" style="position: absolute; left: 51px; top: 0; cursor: pointer;' +
        ...
        if (table == 'ticket' && columnArray instanceof Array) {
            if (columnArray instanceof Array && columnArray.length == 0) {
                newColumnArray = [{cid: 'last_update', title: 'Last Update', display: 1, cell_id: 'ticket-sort-update',
                    ...
                    {cid: 'operator', title: 'Operator', display: 1}, {cid: 'name', title: 'Name', display: 1}, // tid('operator')
            ...
            if (columnArray instanceof Array && columnArray.length == 0) {
                newColumnArray = [{cid: 'date', title: 'Date', display: 1}, {cid: 'chat_id', title: 'Chat ID', display: 1}, // t('Date'), t('Chat ID')
                    {cid: 'name', title: 'Name', display: 1}, {cid: 'operator', title: 'Operator', display: 1}, // t('Name'), tid('operator')
            ...
            if (columnArray instanceof Array && columnArray.length == 0) {
                newColumnArray = [{cid: 'waiting_time', title: 'Waiting Time', display: 1},{cid: 'status', title: 'Status', display: 1}, // t('Status')
                    {cid: 'chat_id', title: 'Chat ID', display: 1, sort: 1, cell_style:'min-width:75px;'}, {cid: 'type', title: 'Type', display: 1}, // t('Chat ID'), t('Type')
                    {cid: 'duration', title: 'Duration', display: 1}, {cid: 'start_time', title: 'Start Time', display: 1}, // t('Duration'), t('Start Time')
                    {cid: 'name', title: 'Name', display: 1}, // t('Waiting Time'), t('Name')
...
ChatDisplayHelperClass.prototype.createGeotrackingFootline = function() {
    ...
    var gtFootlineHtml = '<span style="float: left;">' +
        this.createButton('smartmap-map', 'map-button map-type-button', 'setMapType(\'SMARTMAP\')', t('Normal'), '', 'lr', {'margin-left': '4px','margin-right': '3px'}, t('Normal map'), 12) +
        this.createButton('satellite-map', 'map-button map-type-button', 'setMapType(\'SATELLITE\')', t('Satellite'), '', 'lr', {}, t('Satellite map'), 12) +
        this.createButton('map-visitor-info', 'map-button' + disabledClass, 'selectView(\'external\');showVisitorInfo(\'' + visitorId + '\');', '', '<i class="fa fa-info"></i>', 'lr', {'margin-left': '3px','margin-right': '3px'}, t('Show visitor information')) +
        '</span><span style="float: right;">' +
        this.createButton('map-zoom-in', 'map-button', 'zoomMap(1)', '', '<i class="fa fa-search-plus"></i>', 'lr', {}, t('Zoom in')) +
        this.createButton('map-zoom-out', 'map-button', 'zoomMap(-1)', '', '<i class="fa fa-search-minus"></i>', 'lr',{'margin-left': '3px','margin-right': '4px'}, t('Zoom out')) +
        '</span>';
...
ChatDisplayHelperClass.prototype.createInputControlPanel = function(mode, disabledClass) {
    ...
    if (!lzm_chatDisplay.isMobile && !lzm_chatDisplay.isApp) {
        panelHtml += this.createButton('editor-bold-btn', disabledClass, 'lzm_chatInputEditor.bold();', '<span style="font-weight: bold;">B</span>', '', 'lr',{'margin-left': '4px', 'padding-left': '12px', 'padding-right': '12px'}, '', -1) +
            this.createButton('editor-italic-btn', disabledClass, 'lzm_chatInputEditor.italic();', '<span style="font-style: italic;">I</span>', '', 'lr', {'margin-left': '4px', 'padding-left': '12px', 'padding-right': '12px'}, '', -1) +
            this.createButton('editor-underline-btn', disabledClass, 'lzm_chatInputEditor.underline();', '<span style="text-decoration: underline;">U</span>', '', 'lr',{'margin-left': '4px', 'padding-left': '12px', 'padding-right': '12px'}, '', -1);
        if (mode == 'basic')
            panelHtml += this.createButton('editor-html-btn', '', 'lzm_chatInputEditor.showHTML();', 'HTML', '', 'lr',{'margin-left': '4px', 'padding-left': '12px', 'padding-right': '12px'}, '', -1);
    }
    if (mode != 'basic') {
        panelHtml += this.createButton('send-qrd', disabledClass, '', '', '<i class="fa fa-database"></i>', 'lr',{'margin-left': '4px'}, t('Knowledgebase'));
    }
    return panelHtml;
};
...
ChatDisplayHelperClass.prototype.showBrowserNotification = function(params) {
    ...
    if (lzm_chatTimeStamp.getServerTimeString(null, false, 1) - this.showBrowserNotificationTime > 10000) {
        ...
        text = (text.length > 150) ? text.substr(0, 150) + '...' : text;
        ...
        var notificationHtml = '<div id="browser-notification" class="lzm-notification"' + onclickAction + '>' +
            ...
            '<div style="top:5px;font-weight: bold;">' + lzm_commonTools.htmlEntities(subject) + '</div>' +
            '<div style="top:23px;">' + lzm_commonTools.htmlEntities(sender) + '</div>' +
            '<div style="top:42px;font-style:italic;">' + lzm_commonTools.htmlEntities(text) + '</div>' +
            '</div>' +
...
ChatDisplayHelperClass.prototype.createButton = function(myId, myClass, myAction, myText, myIcon, myType, myCss, myTitle, myTextLength, myLayoutType) {
    return lzm_inputControls.createButton(myId, myClass, myAction, myText, myIcon, myType, myCss, myTitle, myTextLength, myLayoutType);
};
