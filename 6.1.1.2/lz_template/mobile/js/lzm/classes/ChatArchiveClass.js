...
ChatArchiveClass.prototype.updateArchive = function() {
    ...
    if ($('#matching-chats-inner').length == 0) {
        ...
        $('#matching-chats-inner').html(that.createArchiveHtml(chatArchive.chats, selectedChatId, true));
...
ChatArchiveClass.prototype.showArchivedChat = function(chats, cpId, cpName, chatId, chatType) {
    ...
    var footerString = lzm_displayHelper.createButton('cancel-matching-chats', '', '', t('Close'), '', 'lr',{'margin-left': '4px'},'',30,'d');
    var bodyString = '<div id="matching-chats-placeholder"></div>';
    var tableString = '<div class="lzm-dialog-headline3">' +
        lzm_displayHelper.createButton('link-with-ticket', '', '', t('Link with Ticket'),'<i class="fa fa-link"></i>', 'lr', {float:'right','margin-left': '4px','margin-right': '4px'}) +
        lzm_displayHelper.createButton('send-chat-transcript', '', '', t('Send transcript to...'),'<i class="fa fa-mail-forward"></i>', 'lr', {float:'right','margin-left': '4px'}) +
        '</div>' + that.createMatchingChats(chatId) +
        '<div style="position:static;height:1px;" class="lzm-dialog-headline2" data-role="none"></div>' +
        '<div id="chat-content-inner-div"><div style="margin-top: 5px;" data-role="none" id="chat-content-inner">' +
        '<span>' + t('Text') + '</span></div></div>';
...
ChatArchiveClass.prototype.createArchiveHeaderControls = function(page, amount, amountPerPage, totalAmount, filter, query) {
    ...
    controlHtml += '<span style="float: right; margin-right: 129px; padding-top: 4px;">' +
        lzm_displayHelper.createButton('archive-filter', '', 'openArchiveFilterMenu(event, \'' + filter + '\')', t('Filter'), '<i class="fa fa-filter"></i>', 'lr', {'margin-right': '8px'}, '', 10) + '</span>' +
...
ChatArchiveClass.prototype.createMatchingChats = function(chatId) {
    var that = this;
    var matchingChatsHtml = '<div id="matching-chats-inner-div"><div data-role="none" id="matching-chats-inner">' +
...
ChatArchiveClass.prototype.createArchiveContextMenu = function(myObject) {
    ...
    contextMenuHtml += '<div' + disabledClass + ' onclick="showTicketDetails(\'\', false, \'\', \'' + myObject.cid + '\');">' +
        '<i class="fa fa-plus"></i>' +
        '<span id="archive-create-ticket" class="cm-line cm-click cm-line-icon-left">' +
        t('Create Ticket') + '</span></div>';
    contextMenuHtml += '<div onclick="showArchivedChat(\'' + qId + '\', \'' + name + '\', \'' + myObject.cid + '\', \'' + myObject.t + '\');">' +
        '<span id="archive-show-chats" class="cm-line cm-click">' +
        t('All Chats of this User') + '</span></div><hr />';
    contextMenuHtml += '<div onclick="printArchivedChat(\'' + myObject.cid + '\');">' +
        '<i class="fa fa-print"></i>' +
        '<span id="archive-print-chat" class="cm-line cm-click cm-line-icon-left">' +
