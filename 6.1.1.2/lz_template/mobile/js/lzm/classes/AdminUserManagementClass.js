...
AdminUserManagementClass.prototype.saveGroup = function() {
    ...
    $('.operator-priority-input input').each(function() {
...
AdminUserManagementClass.prototype.validateConfigForm = function(type, opOrGr) {
    ...
    if (!formIsValid) {
        ...
        lzm_commonDialog.createAlertDialog(alertMessage, [{id: 'yes', name: t('Yes')}, {id: 'no', name: t('No')}],false,false,true);
...
AdminUserManagementClass.prototype.createAccountConfiguration = function(operator, type) {
    ...
    contentHtml += lzm_inputControls.createInput('operator-uid', 'umg-edit-text-input' + opUidDisabled, opUid, t('Username (ID)'), '<i class="fa fa-barcode"></i>', 'text', 'a');
    contentHtml += lzm_inputControls.createInput('operator-fn', 'umg-edit-text-input', opName, t('Full Name'), '<i class="fa fa-user"></i>', 'text', 'a');
    contentHtml += lzm_inputControls.createInput('operator-em', 'umg-edit-text-input', opEmail, t('Email'), '<i class="fa fa-envelope-o"></i>', 'text', 'a');
    contentHtml += lzm_inputControls.createInput('operator-desc', 'umg-edit-text-input', opDescription, t('Description'), '<i class="fa fa-info"></i>', 'text', 'a');
    contentHtml += '<div><label for="operator-lng">' + t('Native Language') + '</label></div>' +
        lzm_inputControls.createSelect('operator-lng', 'umg-edit-select' + langDisabled, '', true, {position: 'right', gap: '0px'}, {}, '', langArray, opLanguage.toLowerCase(), '');
    contentHtml += '<div style="margin-top: 10px;"><input type="checkbox" class="checkbox-custom" id="operator-inactive"' + opIsInactive + ' />' +
        '<label for="operator-inactive" class="checkbox-custom-label">' + t('Deactivate account') + '</label></div>';
    contentHtml += '</fieldset>';

    if (type != 'bot') {
        contentHtml += '<fieldset id="op-account-auth" class="lzm-fieldset top-space">' +
            '<legend>' + tid('auth') + '</legend>' +
            lzm_inputControls.createSelect('operator-auth', 'umg-edit-select', '', true, {}, {}, '', [{text:tid('standard'),value:0},{text:'LDAP',value:1}], opLDAP, '') +
            '</fieldset>';
    }
    ...
    if ((operator != null && operator.isbot != 1) || type != 'bot') {
        ...
        contentHtml += '<fieldset id="op-account-password" class="lzm-fieldset top-space">' +
            ...
            lzm_inputControls.createInput('operator-pwd', 'umg-edit-text-input', oldPasswd, t('Password'), '<i class="fa fa-lock"></i>', 'password', 'a') +
            lzm_inputControls.createInput('operator-repeat-pwd', 'umg-edit-text-input', oldPasswd, t('Repeat'), '<i class="fa fa-lock"></i>', 'password', 'a') +
            '<div><input' + forceOpPasswordChange + ' type="checkbox" id="operator-force-pwd-change" class="checkbox-custom" />' +
...
AdminUserManagementClass.prototype.createPictureConfiguration = function(operator,isBot) {
    ...
    var contentHtml = '<fieldset class="lzm-fieldset op-config-fs" id="op-picture-configuration"><legend>' + t('Picture') + '</legend>' + opPicture +
        ...
        lzm_inputControls.createInput('operator-pic', 'umg-edit-text-input', '', t('From file'), '<i class="fa fa-file-o"></i>', 'file', 'a') +
...
AdminUserManagementClass.prototype.createBotModeConfiguration = function(operator) {
    ...
    var contentHtml = '<fieldset class="lzm-fieldset op-config-fs" id="op-bot-mode-configuration"><legend>' + t('Bot Mode') + '</legend>' +
        ...
        lzm_inputControls.createSelect('wmm-fwd-after', 'umg-edit-select', '', true, {position: 'right', gap: '0px'}, {}, '', fwdArray, wmohca, '') + '</div>' +
...
AdminUserManagementClass.prototype.createOperatorChatsConfiguration = function(operator) {
    ...
    var contentHtml = '<fieldset class="lzm-fieldset op-config-fs" id="op-chats-configuration">' +
        ...
        lzm_inputControls.createSelect('op-limit-action', 'umg-edit-select', '', true, {position: 'right', gap: '0px'}, {}, '', limitActionsArray, selectedLimitAction, '') + '</div>' +
        '</div>' +
        '</fieldset>';
            ...
            for (var i=0; i<thisTab.perms.length; i++) {
                if (thisTab.perms[i].type == 'select') {
                    ...
                    tab.content += '<label for="permtab-' + thisTab.id + '-select-' + thisTab.perms[i].id + '">' + thisTab.perms[i].text + '</label></div>' +
                        lzm_inputControls.createSelect('permtab-' + thisTab.id + '-select-' + thisTab.perms[i].id, '', '',selectedText, {position: 'right', gap: '0px'}, {'margin-bottom': '20px'}, '', thisTab.perms[i].options, thisTab.perms[i].setting, '');
...
AdminUserManagementClass.prototype.createTextAndEmailsConfiguration = function(opOrGr) {
    ...
    var contentHtml = '<fieldset class="lzm-fieldset op-config-fs" id="op-text-emails-configuration">' +
        ...
        '<div id="text-emails-list-div"><table class="visitor-list-table alternating-rows-table" id="text-emails-list-table">' +
        '<tbody>';
    ...
    contentHtml += lzm_inputControls.createButton('def-text-emails-btn', 'ui-disabled text-emails-edit-btns', 'setTextEmailsAsDefault()', t('Set as default'), '', 'lr',
        {'padding-left': '12px', 'padding-right': '12px'}, t('Set selected texts as default'), 20, 'b');
    contentHtml += '</div>' +
        '</fieldset>' +
...
AdminUserManagementClass.prototype.createTextEmailsInput = function(textAndEmails, aw, edit) {
    ...
    var inputForm = '<div id="text-emails-inner-div" data-aw="' + aw + '" data-edit="' + edit + '">' +
        ...
        lzm_inputControls.createSelect('text-emails-lang-select', disabledClass, '', selectText, {position: 'right', gap: '0px'}, {}, '', optionList, selectedLanguage.toLowerCase(), '') +
        '</fieldset>' +
        '<div id="text-emails-edit-placeholder"></div>' +
    ...
    var textTabContent = '<div id="text-emails-inner-text-placeholder"></div>' +
        '</fieldset>';
    ...
    if (this.selectedListTab == 'group') {
        var emailTabContent = '<div id="text-emails-inner-emails-placeholder"></div>' +
            '</fieldset>';
...
AdminUserManagementClass.prototype.createGroupDetailsConfiguration = function(group) {
    ...
    var contentHtml = '<fieldset class="lzm-fieldset">' +
        ...
        lzm_inputControls.createInput('group-id', 'umg-edit-text-input' + idDisabled, groupId, t('Group-ID'), '<i class="fa fa-barcode"></i>', 'text', 'a') +
        lzm_inputControls.createInput('group-email', 'umg-edit-text-input', groupEmail, t('Group-Email (visible to all users)'), '<i class="fa fa-envelope-o"></i>', 'text', 'a') +
        '<div class="top-space">' +
        '<input type="checkbox" class="checkbox-custom" id="group-is-default"' + defaultIsChecked + ' />' +
        '<label for="group-is-default" class="checkbox-custom-label checkbox-custom-label-parent" id="group-is-default">' + t('Default Group') + '</label>' +
        ...
        '<label for="group-interactions-visitors" class="checkbox-custom-label checkbox-custom-label-parent" id="group-interactions-visitors">' + t('Allow Interactions with Visitors / Customers') + '</label>' +
        '</div><div class="left-space-child">' +
        ...
        '<label for="group-interactions-operators" class="checkbox-custom-label checkbox-custom-label-parent" id="group-interactions-operators">' + t('Allow Interactions with Operators / Groups') + '</label>' +
        '</div><div class="left-space-child">' +
        ...
        '<div id="group-title-div" class="top-space"><table class="visitor-list-table alternating-rows-table" id="group-title-table">';
    ...
    contentHtml += lzm_inputControls.createButton('new-title-btn', '', 'createGroupTitle()', t('New'), '', 'lr',{'margin-right': '5px', 'padding-left': '12px', 'padding-right': '12px'}, t('Create new signature'), 20, 'b');
    contentHtml += lzm_inputControls.createButton('edit-title-btn', 'ui-disabled title-edit-btns', 'editGroupTitle()', t('Edit'), '', 'lr',{'margin-right': '5px', 'padding-left': '12px', 'padding-right': '12px'}, t('Edit selected signature'), 20, 'b');
    contentHtml += lzm_inputControls.createButton('rm-title-btn', 'ui-disabled title-edit-btns', 'removeGroupTitle()', t('Remove'), '', 'lr',{'padding-left': '12px', 'padding-right': '12px'}, t('Remove selected signature'), 20, 'b');
    contentHtml += '</div>' +
        '</fieldset>';
...
AdminUserManagementClass.prototype.createGroupTitleInput = function(titles, lang) {
    ...
    var inputForm = '<div id="group-title-inner-div"><fieldset class="lzm-fieldset input-fs" id="group-title-inner-fs">' +
        ...
        lzm_inputControls.createSelect('gr-title-lang', langDisabled, '', langText, {position: 'right', gap: '0px'}, {}, '', langList, lang, '') +
        '</div></div>' +
...
AdminUserManagementClass.prototype.createGroupChatsConfiguration = function(group) {
    ...
    var contentHtml = '<fieldset class="lzm-fieldset"><legend>' + t('Concurrent Chat Amount') + '</legend>' +
        ...
        lzm_inputControls.createSelect('gr-limit-action', '', '', limitActionText, {position: 'right', gap: '0px'}, {}, '', limitActionList, selectedLimitAction, '') +
        '</div></div></div>' +
        '</fieldset>' +
        ...
        lzm_inputControls.createSelect('gr-transcript-email', '', '', transcriptEmailText, {position: 'right', gap: '0px'}, {}, '', transcriptEmailList, selectedTranscriptEmail, '') +
        '</div></div>' +
        '</fieldset>' +
        '<fieldset class="lzm-fieldset top-space"><legend>' + t('Operator Priority') + '</legend>' +
        '<div id="group-priorities-list-div">' +
        '<table class="visitor-list-table alternating-rows-table"><thead>' +
    ...
    for (i=0; i<operators.length; i++) {
        if ($.inArray(groupId, operators[i].groups) != -1 && operators[i].isbot != 1) {
            ...
            contentHtml += '<tr><td>' + operators[i].userid + '</td>' +
                ...
                lzm_inputControls.createInput('op-prio-' + operators[i].id,'operator-priority-input',opPrio,'','','number','','data-op-id="' + operators[i].id + '"') +
                '</td></tr>';
        }
    }
    contentHtml += '</tbody></table>' +
...
AdminUserManagementClass.prototype.createGroupInputFieldConfiguration = function(group) {
    ...
    var createContentTable = function(cot, tableData) {
        ...
        for (var i=0; i<tableData.length; i++) {
            ...
            tableHtml += '<tr>' +
                ...
                '<td style="text-align: center;">' +
                lzm_inputControls.createSelect('gr-ift-masked-' + cot + '-' + tableData[i].id, 'gr-input-field-select', '', selText, {position: 'right', gap: '0px'}, {}, '', maskList, selMask, '',  {'input-id': tableData[i].id}) +
                '</td>' +
                '</tr>';
        }
    ...
    var chatTabContent = '<fieldset class="lzm-fieldset" id="gr-input-fields-chat-fs"><legend>' + t('Chat / Callback Login Inputs') + '</legend>' +
        '<div id="input-fields-chat-list-div">' + createContentTable('chat', inputList) +
        '</div></fieldset>';
    var ticketTabContent = '<fieldset class="lzm-fieldset" id="gr-input-fields-ticket-fs"><legend>' + t('Ticket Inputs') + '</legend>' +
        '<div id="input-fields-ticket-list-div">' + createContentTable('ticket', inputList) +
        '</div></fieldset>';
...
AdminUserManagementClass.prototype.createGroupTicketsConfiguration = function(group, selectedTab) {
    ...
    var outMbText = (typeof lzm_pollServer.globalConfig.site[0].dbconf.glEmail[selOutMb] != 'undefined') ? lzm_pollServer.globalConfig.site[0].dbconf.glEmail[selOutMb].e : firstOutMb;
    ...
    var emailTabContent = '<fieldset class="lzm-fieldset" id="gr-tickets-email-out-fs"><legend>' + t('Outgoing Emails') + '</legend>' +
        ...
        lzm_inputControls.createSelect('gr-tickets-out-mb', '', '', outMbText, {position: 'right', gap: '0px'}, {}, '', outMbList, selOutMb, '') +
        '</div>' +
        '</fieldset>' +
        '<fieldset class="lzm-fieldset" id="gr-tickets-email-in-fs" style="margin-top: 10px;"><legend>' + t('Incoming Emails') + '</legend>' +
        '<label for="gr-ticket-in-action">' + t('Download / receive emails from:') + '</label><div id="gr-ticket-in-mb-div">'+
    ...
    emailTabContent += '</tbody></table>' +
        ...
        lzm_inputControls.createSelect('gr-ticket-in-action', '', '', inActionText, {position: 'right', gap: '0px'}, {}, '', inActionList, selInAction, '') +
        '</div></div>' +
        '</fieldset>';
    ...
    for (i=0; i<operators.length; i++) {
        if ($.inArray(groupId, operators[i].groups) != -1) {
            ...
            routingTabContent += '<tr id="gr-ticket-assign-list-line-' + operators[i].id + '" class="gr-ticket-assign-list-line lzm-unselectable">' +
                '<td>' + operators[i].userid + '</td>' +
                '<td>' + lzm_inputControls.createSelect('gr-taa-' + operators[i].id, 'gr-ticket-assign-select','', assText, {position: 'right', gap: '0px'}, {}, '', assList, selAss, '') + '</td>' +
                '</tr>';
        }
    }
...
AdminUserManagementClass.prototype.createGroupTicketsSocialMediaTab = function(socialMediaList) {
    var socialTabContent = '<fieldset class="lzm-fieldset" id="gr-tickets-social-fs"><legend>' + t('Social Media Channels') + '</legend>' +
        '<div id="gr-ticket-social-list-div">' +
...
AdminUserManagementClass.prototype.createSocialMediaChannel = function(smc) {
    ...
    var inputForm = '<div id="social-media-inner-div"><fieldset class="lzm-fieldset input-fs" id="social-media-inner-fs">' +
        ...
        '<div style="margin-bottom: 10px;">' +
        '<div><label for="smc-type">' + t('Language') + '</label></div><div>' +
        lzm_inputControls.createSelect('smc-type', typeSelectDisabled, '', typeList[selType], {position: 'right', gap: '0px'}, {}, '', typeList, selType, '') +
        '</div></div>' +
        ...
        '<div style="margin-bottom: 10px;">' +
        '<div><label for="smc-content-type">' + t('Language') + '</label></div><div>' +
        lzm_inputControls.createSelect('smc-content-type', typeSelectDisabled, '', contentTypeList[selContentType], {position: 'right', gap: '0px'}, {}, '',  contentTypeList, selContentType, '') +
    ...
    if (smc != null && smc.t == 7) {
        inputForm += '<fieldset class="lzm-fieldset"><legend>' + t('Tweets') + '</legend>' +
            '<div style="margin-top: 10px;" class="ui-disabled"><input type="checkbox" class="checkbox-custom" id="smc-use-me" checked="checked" />' +
            '<label for="smc-use-me" class="checkbox-custom-label">' + t('Create tickets from every tweet mentioning me') + '</label></div>' +
            '<div style="margin-top: 10px;"><input type="checkbox" class="checkbox-custom" id="smc-use-track"' + useTrack + ' />' +
            '<label for="smc-use-track" class="checkbox-custom-label">' + t('Create tickets from tweets containing one of these hashtags (comma separated)') + '</label></div>' +
...
AdminUserManagementClass.prototype.createOpeningHours = function(oh) {
    ...
    var inputForm = '<div id="opening-hours-inner-div"><fieldset class="lzm-fieldset input-fs" id="opening-hours-inner-fs">' +
        ...
        lzm_inputControls.createSelect('oh-weekday', '', '', weekDayList[0].text, {position: 'right', gap: '0px'}, {}, '', weekDayList, 0, '') +
...
AdminUserManagementClass.prototype.createContextMenuContent = function(tab, myObject) {
    ...
    contextMenuHtml += '<div' + disabledClass + ' onclick="createListObject(null);">' +
        ...
        '<span id="umg-ctxt-new" class="cm-line cm-line-icon-left cm-click">' +
        t('New') + '</span></div><hr />';
    disabledClass= (false) ? ' class="ui-disabled"' : '';
    contextMenuHtml += '<div' + disabledClass + ' onclick="editListObject(\'' + myObject.id + '\', event);">' +
        '<i class="fa fa-edit"></i>' +
        '<span id="umg-ctxt-edit" class="cm-line cm-line-icon-left cm-click">' +
        t('Edit') + '</span></div>';
    disabledClass= (false) ? ' class="ui-disabled"' : '';
    contextMenuHtml += '<div' + disabledClass + '>' +
        '<i class="fa fa-remove"></i>' +
        '<span id="umg-ctxt-rm" class="cm-line cm-line-icon-left cm-click" onclick="removeListObject(\'' + myObject.id + '\', event);">' +
        t('Remove') + '</span></div>';
    if (this.selectedListTab == 'user')
    {
        contextMenuHtml += '<div' + disabledClass + '>' +
...
AdminUserManagementClass.prototype.createNewObjectMenu = function() {
    var contextMenuHtml = '';
    contextMenuHtml += '<div onclick="createListObject(\'user\');">' +
        '<i class="fa fa-user"></i>' +
        '<span id="umg-new-btn-op" class="cm-line cm-line-icon-left cm-click">' +
        tid('operator') + '</span></div>';
    contextMenuHtml += '<div onclick="createListObject(\'bot\');">' +
        '<span id="umg-new-btn-bot" class="cm-line cm-click">' +
        t('Chat Bot (Virtual Assistant)') + '</span></div><hr />';
    contextMenuHtml += '<div onclick="createListObject(\'group\');">' +
        '<i class="fa fa-users"></i>' +
        '<span id="umg-new-btn-gr" class="cm-line cm-line-icon-left cm-click">' +
        t('Group') + '</span></div>';
    return contextMenuHtml;
};
