...
function logit(myObject, myLevel) {
    ...
    if(debug) {
        ...
        var postUrl = lzm_chatPollServer.chosenProfile.server_protocol + lzm_chatPollServer.chosenProfile.server_url + '/mobile/logit.php?acid=' + acid;
...
function sendChat(chatMessage, chat_reco, translatedChatMessage, visitorChat) {
    ...
    if (lzm_chatServerEvaluation.userChats.getUserChat(lzm_chatDisplay.active_chat) != null ||
        ...
        if (chatMessage != '') {
            ...
            if(!lzm_chatUserActions.messageFromKnowledgebase){
...
function showAllchatsList(userAction) {
    ...
    lzm_chatDisplay.updateChatMembers(null);
...
function toggleMemberList(){
    ...
    lzm_chatDisplay.updateChatMembers(/*lzm_chatDisplay.getUserObjectFromChat(lzm_chatServerEvaluation.userChats.getUserChat(lzm_chatUserActions.active_chat_reco))*/);
}
...
function leaveChat(chatId) {
    ...
    if (lzm_chatDisplay.thisUser.b_id != '') {
        ...
        var chatObj = lzm_chatServerEvaluation.userChats.getUserChat(lzm_chatDisplay.active_chat_reco);
        if (chatObj != null) {
            var isMainChat = true;
            var chatServerAccepted = (typeof lzm_chatDisplay.thisUser.b_chat.pn != 'undefined' && lzm_chatDisplay.thisUser.b_chat.pn.acc == 1);
            var chatLocalAccepted = ($.inArray(lzm_chatDisplay.thisUser.b_chat.id, lzm_chatUserActions.localAcceptedChats) != -1);
            ...
            var chatHasEnded = lzm_chatServerEvaluation.userChats.getUserChat(lzm_chatDisplay.active_chat_reco).status == 'left' || lzm_chatDisplay.thisUser.is_active == false;
            for (var l=0; l<chatMember.length; l++) {
                if (/*chatAccepted && */chatMember.length > 1 && chatMember[l].st != 0) {
            ...
            if(!chatLocalAccepted && !chatServerAccepted /*&& !lastOperator*/ && !chatHasEnded && !chatDeclined){
                lzm_chatUserActions.declineChat(lzm_chatDisplay.thisUser.id, lzm_chatDisplay.thisUser.b_id, lzm_chatDisplay.thisUser.b_chat.id);
            }
...
function showNextChat(){
    if(lzm_chatDisplay.active_chat == 'LIST'){
        showAllchatsList();
        return false;
    }
...
function getNextChatInRow() {
    ...
    var chatList = Object.keys(lzm_chatServerEvaluation.userChats.getUserChatList());
    var newActiveChat = 'LIST', myChat = null;
    for (var j=(chatList.length - 1); j>=0; j--) {
        myChat = lzm_chatServerEvaluation.userChats.getUserChat(chatList[j]);

        if(lzm_chatServerEvaluation.userChats.isInPublicGroupChat(myChat))
        {
            continue;
        }

        if (myChat != null && $.inArray(chatList[j], lzm_chatDisplay.closedChats) == -1 &&
            ...
            newActiveChat = chatList[j];
            break;
        }
    }
    myChat = null;
    if (newActiveChat == 'LIST') {
        for (var k=(chatList.length - 1); k>=0; k--) {
            myChat = lzm_chatServerEvaluation.userChats.getUserChat(chatList[k]);
            ...
            if (myChat != null && myChat.type == 'external' &&
                $.inArray(chatList[k], lzm_chatDisplay.closedChats) == -1 &&
                typeof myChat.my_chat != 'undefined' && myChat.my_chat) {
                newActiveChat = chatList[k];
                break;
            }
        }
    }
...
function openAllChatsFilterMenu(e) {
    ...
    if (lzm_chatDisplay.allchatsDisplay.showAllchatsFilterMenu) {
        ...
        var xValue = $('#chat_page').width() + 4;
        var yValue = $('#chat_page').height() - 22;
    ...
    for (var i=0; i<languageStrings.length; i++) {
        if (languageStrings[i].key == myKey) {
            ...
            var inputFieldHtml = '<input type="text" id="translation-string-input"' +
                ...
                ' data-role="none" class="lzm-text-input" style="min-width: 0px; width:100%;"/>';
...
function saveLinkGeneratorCode(){
    lzm_chatDisplay.LinkGenerator.SaveCodeToServer(false,false,true);
}

function loadLinkGeneratorCode(){
...
function selectView(id,required) {
    ...
    if (id != lzm_chatDisplay.selected_view || required) {
        ...
        if (oldSelectedView == 'qrd') {
            cancelQrdPreview();
            /*
            lzm_chatDisplay.resourcesDisplay.CacheTree = $('#qrd-tree-body').html();
            //$('#qrd-tree-body').remove();
            $('#preview-qrd').remove();
            $('#send-qrd-preview').remove();
            $('#qrd-tree-headline').remove();
            */
        }
...
function switchTicketListPresentation(ticketFetchTime, counter, ticketId) {
    ...
    if (counter == 0) {
        if ($('#matching-tickets-table').length == 0) {
            ...
            $('#matching-ticket-list-loading').css({position: 'absolute', left:0,top:0,bottom:0,right:0,'background-color': '#ffffff','background-position': 'center', 'z-index': 1000, opacity: 1});
        }
    }
...
function showTicketMsgTranslator(ticketId, msgNo) {
    ...
    if(msgNo==-1)
        for (i=0; i<lzm_chatDisplay.ticketListTickets.length; i++) {
            if (lzm_chatDisplay.ticketListTickets[i].id == ticketId) {
                msgNo = lzm_chatDisplay.ticketListTickets[i].messages.length - 1;
            }
        }
...
function selectTicket(ticketId, noUserInteraction, inDialog) {
    ...
    if (!inDialog) {
        ...
        messageText = '';
        try {
            messageText = lzm_commonTools.htmlEntities(ticket.messages[ticket.messages.length - 1].mt).replace(/\r\n/g, '\n').replace(/\r/g, '\n').replace(/\n/g, '<br />');
        } catch (e) {}
        try {
            $('#ticket-content-inner').html(messageText);
        } catch(e) {}
    }
}
...
function previewTicketAttachment(url) {
    var height = $('#ticket-attachment-list').height()-$('#attachment-table').height();
    if(url!=null)
        $('#att-img-preview-field').css({'height':height+'px','background-image':'url(\''+url+'\')'});
    else
        $('#att-img-preview-field').css({'background-image':'none'});
}

function saveTicketDetails(ticket, channel, status, group, editor, language, name, email, company, phone, message, attachments, comments, customFields, subStatus, subChannel, chat, mc, subject) {
    ...
    lzm_chatServerEvaluation.expectTicketChanges = true;
    lzm_chatUserActions.saveTicketDetails(ticket, channel, status, group, editor, language, name, email, company, phone, message, attachments, comments, customFields, subStatus, subChannel, chat, mc, subject);
}
...
function saveTicketTranslationText(myTicket, msgNo, text, type) {
    if (typeof type == 'undefined' || type != 'comment') {
        if (myTicket != null) {
            ...
            saveTicketDetails(myTicket, myTicket.t, ticketStatus, ticketGroup, ticketOperator, myTicket.l, null, null, null, null, null, null, null, null, null, null, null, mc);
        }
    } else {
        if (myTicket != null) {
...
function changeTicketStatus(myStatus, mySubStatus, myChannel, mySubChannel, fromKey) {
    ...
    if (lzm_chatDisplay.selectedTicketRow != '') {
        ...
        } else
        {
            ...
            if (!fromKey) {
                saveTicketDetails(myTicket, newChannel, /*'change~' + myStatus*/newStatus, ticketGroup, ticketEditor, myTicket.l, '', '', '', '', '',null,null,null,newSubStatus,newSubChannel);
            } else {
...
function handleTicketTreeClickEvent(_id,_parent,_subStatus){
    ...
    if(typeof _subStatus != 'undefined')
    ...
    lzm_chatPollServer.ticketFilter = f;
...
function addTicketToWatchList(_ticketId,_operatorId){
    lzm_chatUserActions.addTicketToWatchList([{id: _ticketId,operatorId: _operatorId}]);
}

function removeTicketFromWatchList(_ticketId){
    lzm_chatUserActions.removeTicketFromWatchList([{id: _ticketId}]);
}
...
function selectArchivedChat(chatId, inDialog) {
    ...
    if (inDialog) {
        ...
        try {
            chatHtml = '' +
                '<div style="margin-left: -10px;">' + thisChat.chtml.replace(/\.\/images\//g, 'img/') + '</div>';
        } catch(e) {
            chatHtml = '';
        }
        if (chatId != '') {
...
function removeFromDynamicGroup(id, groupId) {
    ...
    if (lzm_commonPermissions.checkUserPermissions(lzm_chatDisplay.myId, 'group', '', lzm_chatServerEvaluation.groups.getGroup(groupId))) {
        ...
        if (!isGroupOwner) {
            ...
            if(visitorBrowser != null && visitorBrowser[0] != null && typeof visitorBrowser[0].b_chat.dgc != 'undefined'){
                visitorBrowser[0].b_chat.dgc = '';
            }
...
function selectOperatorLine(id, lineCounter, userid, name, fromOpList) {
    ...
    if (!internalChatsAreDisabled && !lzm_chatDisplay.isMobile && !lzm_chatDisplay.isApp && lastOpListClick[0] == id && now - lastOpListClick[1] < 500 &&
        ...
        setTimeout(function() {
            ...
            $('#operator-list-line-' + lineId).addClass('selected-table-line');
        }, 1);
    }
    } catch(ex) {}
}
