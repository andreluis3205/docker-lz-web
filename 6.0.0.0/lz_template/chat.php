...
if(!isset($_GET[GET_EXTERN_TEMPLATE]))
{
    ...
    $html = str_replace("<!--show_oib-->",To::BoolString(!empty(Server::$Configuration->File["gl_soib"])),$html);
    ...
    $html = str_replace("<!--is_small-->",To::BoolString(!empty($_GET["s"]) || (empty(Server::$Configuration->File["gl_cali"]) && empty(Server::$Configuration->File["gl_cahi"]))),$html);
	...
	$html = str_replace("<!--info_text-->",base64_encode(Server::$Configuration->File["gl_info"]),$html);
    ...
    $html = str_replace("<!--function_callback-->",To::BoolString(!empty($_GET["cmb"]) || !empty($_GET["ofc"])),$html);
    ...
    $html = str_replace("<!--hide_group_select_chat-->",To::BoolString(Communication::GetParameter("hcgs",0,$nu,FILTER_VALIDATE_INT)=="1"),$html);
...
else
{
	...
	else if($_GET[GET_EXTERN_TEMPLATE] == "lz_chat_frame_chat")
	{
        ...
        if(!empty($groupid) && isset(Server::$Groups[$groupid]))
        {
            ...
            $html = str_replace("<!--FU_HIDDEN-->",((empty(Server::$Groups[$groupid]->ChatFunctions[5])) ? "none" : ""),$html);
