...
class GroupBuilder
{
	...
	function Generate($_user=null,$_allowBots=false)
	{
		...
		{
			if($internaluser->LastActive > (time()-Server::$Configuration->File["timeout_clients"]) && $internaluser->Status < USER_STATUS_OFFLINE && ($_allowBots || !$internaluser->IsBot) && !$internaluser->MobileSleep())
			{
				...
				for($count=0;$count<count($igroups);$count++)
				{
                    ...
                    {
                        if($internaluser->GetMaxChatAmountStatus(Server::$Groups[$igroups[$count]]) != USER_STATUS_AWAY)
...
class ChatRouter
{
    ...
    function FindOperator($_user,$_allowBots=false,$_requireBot=false,$_exclude=null)
    {
        ...
        if($fromDepartment && sizeof($available_internals) > 0)
        {
            ...
            if(!$this->IsPredefined && Server::$Configuration->File["gl_alloc_mode"] != ALLOCATION_MODE_ALL || $fromCookie == $matching_internal || Server::$Operators[$matching_internal]->IsBot)
