...
class UserGroup extends BaseUser
{
    ...
    static function PersistentJoin($_userId,$_systemId,$joined=false)
    {
        if(UserGroup::IsDynamicGroup())
        {
            ...
            {
                $tgroup = Communication::GetParameter(GET_EXTERN_DYNAMIC_GROUP,"");
                if(isset(Server::$Groups[$tgroup]))
                {
                    Server::$Groups[$tgroup]->AddMember($_systemId,true);
                    Server::$Groups[$tgroup]->LoadMembers();
                    $joined = true;
                }
            }
            else
