	...
	if(!(IS_FILTERED && !FILTER_ALLOW_CHATS))
	{
		...
		if($USER->Browsers[0]->Status == CHAT_STATUS_OPEN)
		{
			...
			else if((count(VisitorChat::$Router->OperatorsAvailable) + VisitorChat::$Router->OperatorsBusy) > 0)
			{
                ...
                login(Server::$Groups[$USER->Browsers[0]->DesiredChatGroup]);
				$USER->Browsers[0]->SetWaiting(!($chatPosition == 1 && count(VisitorChat::$Router->OperatorsAvailable) > 0 && !(!empty($USER->Browsers[0]->DesiredChatPartner) && Server::$Operators[$USER->Browsers[0]->DesiredChatPartner]->Status == USER_STATUS_BUSY)));
...
function login($_group)
{
    ...
    $USER->Browsers[0]->UserData->LoadFromLogin($_group);

    if(!empty($_POST["p_cmb"]))
    {
        $USER->Browsers[0]->CallMeBack = true;
        $USER->Browsers[0]->SetCallMeBackStatus(1);
    }
...
function processTyping($_dgroup="")
{
		...
		if($group->IsDynamic && isset($group->Members[$USER->Browsers[0]->SystemId]))
		{
			...
			foreach($group->Members as $member => $persistent)
				if($member != $USER->Browsers[0]->SystemId)
				{
                    ...
                    {
						$name = CacheManager::GetValueBySystemId($member,"fullname",LocalizationManager::$TranslationStrings["client_guest"]);
                        if(empty($name))
	...
	foreach($USER->Browsers[0]->Members as $sysid => $chatm)
		if($chatm->Status < 2 && empty($chatm->Declined))
			$USER->AddFunctionCall("lz_chat_set_room_member('".base64_encode($sysid)."','".base64_encode(Server::$Operators[$sysid]->Fullname)."');",false);
...
function beginnConversation()
{
	...
	if(!empty(Server::$Configuration->File["gl_save_op"]))
		Cookie::Set("internal_user",Server::$Operators[$USER->Browsers[0]->OperatorId]->UserId);
	$USER->Browsers[0]->DesiredChatPartner = Server::$Operators[$USER->Browsers[0]->OperatorId]->SystemId;
	$USER->AddFunctionCall("lz_chat_set_status(lz_chat_data.STATUS_ACTIVE);",false);
	$USER->AddFunctionCall("lz_chat_shout(1);",false);
    if($USER->Browsers[0]->CallMeBack == 2)
        $USER->AddFunctionCall("lz_chat_call_back_info('".base64_encode(LocalizationManager::$TranslationStrings["client_thank_you"]." ".str_replace(array("<b>","</b>"),"",str_replace("<!--operator_name-->",Server::$Operators[$USER->Browsers[0]->OperatorId]->Fullname,LocalizationManager::$TranslationStrings["client_now_speaking_to"])."</b>"))."');",false);
    else if($USER->Browsers[0]->CallMeBack > 0)
...
class OverlayChat
{
    ...
    function InitFeedback($_userInitiated=true)
    {
        ...
        {
            if(Feedback::GetByChatId($cid)==null)
            {
