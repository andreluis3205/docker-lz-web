...
function lz_chat_loaded() 
{
	...
	//lz_chat_get_frame_object('lz_chat_transcript_email').disabled = !(lz_chat_data.ExternalUser.Email!='');
...
{
    if(!<!--function_knowledgebase--> || !lz_chat_data.KnowledgebaseSuggest)
        return;
    ...
    if(phrase.length < 1)
        phrase = "%ALL%";
    else if(!_backgroundSearch && phrase.length <= 3)
    {
        lz_chat_dialog("<!--lang_client_search_query_too_short-->",null,null,null,null);
        return;
    }
